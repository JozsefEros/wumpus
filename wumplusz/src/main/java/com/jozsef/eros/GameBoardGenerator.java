...
public class GameBoardGenerator {
    ...
    public void fillBoard() {
        ...
        for (int i = 0; i < InnerWallGenerator.getWall().length; i++)
            for (int j = 0; j < InnerWallGenerator.getWall().length; j++) {
                if (InnerWallGenerator.getWall()[i][j] == 'W' ||
                        InnerWallGenerator.getWall()[i][j] == 'P') {
